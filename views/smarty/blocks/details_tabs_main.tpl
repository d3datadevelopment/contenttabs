    ...
    <div class="tabbedWidgetBox clear">
        <ul id="itemTabs" class="nav nav-tabs">
            [{foreach from=$tabs item="tab" name="tabs"}]
                <li[{if $smarty.foreach.tabs.first}] class="rounded"[{else}] class="rounded" [{/if}]>[{$tab}]</li>
            [{/foreach}]
...
[{oxstyle include=$oViewConf->getModuleUrl("d3contenttabs", "src/css/d3contenttabs.css")}]